...
import { applyScenePhotoConstraint } from "../../services/photoConstraintService";
...
import {
  applyRapierDynamicsSimulation,
  isRapierDynamicsResult,
} from "../../services/rapierDynamicsService";
import {
...
import {
  applyAuthoritativeParticipantSpeedChanges,
} from "../../utils/reconstructionSpeedAuthoring";
import {
  getBezierDisplayPositions,
  getManualBezierSegmentSvgPath,
  getManualBezierSegmentView,
  resetManualBezierSegment,
  setManualBezierSegmentEnabled,
  shiftBezierHandlesWithAnchor,
  updateManualBezierHandle,
...
import "./photoAssistedIntegrated.css";
...
type DragState =
  ...
  | {
      kind: "participant-bezier-handle";
      participantId: string;
      pointId: string;
      handle: "in" | "out";
    }
...
export default function AccidentReconstructionEditor({
  ...
  /*
   * [RoadSafe:RapierDynamicsRunSequenceV1]
   * Prevent a slower, older async Rapier run from overwriting a newer edit/run.
   */
  const rapierPhysicsRunSequenceRef = useRef(0);
  ...
  const latestReconstructionRef = useRef(reconstruction);
  latestReconstructionRef.current = reconstruction;
  ...
  const [rapierPhysicsRunning, setRapierPhysicsRunning] = useState(false);
  const [playbackSpeed, setPlaybackSpeed] = useState(1);
  const [sceneExpanded, setSceneExpanded] = useState(false);
  const [sceneAssetDragActive, setSceneAssetDragActive] = useState(false);
  const [activeReconstructionView, setActiveReconstructionView] =
    useState<"2D" | "3D">("2D");
  const [activeTopWorkspaceTab, setActiveTopWorkspaceTab] =
    useState<"scene" | "photos">("scene");
  ...
  /*
   * Manual Bezier is an authoring mode only. Live GPS tracing and field
   * placement remain separate workflows and are not rewritten by this toggle.
   */
  const [bezierEditMode, setBezierEditMode] =
    useState(false);
  ...
  useEffect(() => {
    ...
    const runSequence = ++rapierPhysicsRunSequenceRef.current;

    livePhysicsTimerRef.current = window.setTimeout(() => {
      livePhysicsTimerRef.current = null;

      void applyRapierDynamicsSimulation(latestReconstructionRef.current)
        .then((simulated) => {
          if (rapierPhysicsRunSequenceRef.current !== runSequence) {
            return;
          }
        ...
        .catch((error) => {
          console.warn(
            "RoadSafe live Rapier physics could not be refreshed; keeping the current reconstruction.",
            error,
          );
        });
    }, 500);
    ...
    return () => {
      ...
      rapierPhysicsRunSequenceRef.current += 1;
    };
  }, [
    isPlaying,
    livePhysicsEnabled,
    physicsInputSignature,
    physicsParticipantCount,
  ]);
  ...
  const updateParticipant = useCallback(
    (participantId: string, updates: Partial<ReconstructionVehicle>) => {
      setReconstruction((current) => {
        ...
        const proposed: AccidentReconstruction = {
          ...current,
          ...
          vehicles: current.vehicles.map((participant) => {
             ...
             *
             * A pure speed edit must not run through normalisePointZRoute first.
             * That normalizer is designed to reconcile route timing and can
             * rescale/retime the route before the authoritative speed pipeline
             * sees the investigator's new value.
             */
            if (
              !speedEdit &&
              (
  ...
  const handleToggleSelectedBezierSegment =
    useCallback(
      () => {
        if (
  ...
  const handleResetSelectedBezierSegment =
    useCallback(
      () => {
        if (
          !selectedParticipantId
  ...
  const addPhotoConstraint = useCallback(
    (constraint: ScenePhotoConstraint) => {
      setReconstruction((current) => ({
        ...current,
        photoConstraints: [
          ...(current.photoConstraints ?? []),
          constraint,
        ],
      }));
    },
  ...
  const updatePhotoConstraint = useCallback(
    (
      constraintId: string,
      ...
      setReconstruction((current) => ({
        ...current,
        photoConstraints: (current.photoConstraints ?? []).map(
          (constraint) =>
  ...
  const deletePhotoConstraint = useCallback(
    (constraintId: string) => {
      setReconstruction((current) => ({
        ...current,
        photoConstraints: (current.photoConstraints ?? []).filter(
  ...
  const handleRunPhysics = useCallback(async (): Promise<void> => {
    const runSequence =
      ++rapierPhysicsRunSequenceRef.current;

    setIsPlaying(false);
    setCurrentTime(0);
    currentTimeRef.current = 0;
    setRapierPhysicsRunning(true);

    showSaveMessage(
      "Running Rapier 3D deterministic dynamics and RoadSafe forensic calculations…",
      "info",
      3200,
    );
    ...
    try {
      const simulated =
        await applyRapierDynamicsSimulation(
          reconstruction,
        );
      ...
      const rapier =
        simulated.lastPhysicsSimulation
          ?.rapierDynamics;

      showSaveMessage(
        rapier?.usedForPostImpactTrajectories
          ? `Rapier dynamics applied. ${rapier.collisionStarts} rigid-body contact pair(s) detected; 2D and 3D now share the Rapier-generated post-impact trajectory.`
          : "Rapier completed, but no rigid-body contact was found in the authored approach window. RoadSafe retained the existing deterministic path; review paths, headings, dimensions and speeds.",
        "info",
        5200,
      );
    } catch (error) {
      console.error(
        "Rapier deterministic dynamics failed:",
        error,
      );

      /*
       * Keep the workstation usable if WASM initialization ever fails. This is
       * an explicit, visible fallback—not a silent replacement.
       */
      const fallback =
        applyPhysicsSimulation(
          reconstruction,
      ...
      if (
        ...
        showSaveMessage(
          "Rapier could not initialize/run, so RoadSafe used the legacy deterministic physics fallback. Check the browser console before relying on this simulation.",
          "error",
          6500,
        );
      }
    ...
    if (
      ...
      try {
        ...
        if (
          ...
          showSaveMessage(
            "Rapier found no closing rigid-body contact. Review approach directions, paths, headings, collider dimensions and speeds.",
            "info",
            4500,
          );
        }
        ...
        console.error(
          "Rapier auto-run before playback failed:",
          error,
        );
        ...
        if (
          ...
          showSaveMessage(
            "Rapier could not prepare playback. RoadSafe used the legacy deterministic fallback; no closing participant contact was found.",
            "error",
            5200,
          );
        }
  ...
  useEffect(() => {
    ...
    const processPointerMove = () => {
      ...
      if (
        dragState.kind ===
        "participant-bezier-handle"
      ) {
        updateBezierHandle(
          dragState.participantId,
          dragState.pointId,
          dragState.handle,
          position,
        );
  ...
  const handleApplyPhotoConstraint = useCallback(
    (constraintId: string) => {
      const constraint = (reconstruction.photoConstraints ?? []).find(
        (item) => item.id === constraintId,
      );

      if (!constraint) {
        showSaveMessage(
          "The selected photo observation no longer exists.",
          "error",
          4200,
        );
        return;
      ...
      try {
        const result = applyScenePhotoConstraint({
          reconstruction,
          constraint,
        });
        ...
        if (
          ...
          constraint.kind === "Scene Object Position" &&
          constraint.sceneObjectId
        ) {
          handleSelectSceneObject(
            constraint.sceneObjectId,
          );
        } else if (
          constraint.kind === "Evidence Position" &&
          ...
          setSelectedPathPointId(null);
          setSelectedSceneObjectId(null);
        } else if (constraint.kind === "Primary Impact Point") {
          const impactTime =
            result.reconstruction.lastPhysicsSimulation
              ?.primaryImpactTimeSeconds ??
            Math.min(
              ...result.reconstruction.vehicles
                .map((participant) => participant.collisionTimeSeconds)
                .filter(Number.isFinite),
              result.reconstruction.durationSeconds / 2,
            );
          ...
          if (Number.isFinite(impactTime)) {
            setCurrentTime(
              Math.max(
                0,
                Math.min(
                  result.reconstruction.durationSeconds,
                  impactTime,
                ),
              ),
            );
        ...
        showSaveMessage(
          `${result.message} Main Reconstruction updated. Physics is now DIRTY where geometry changed; run Physics again before relying on the previous simulation result.`,
          "info",
          5600,
        );
      } catch (error) {
        showSaveMessage(
          error instanceof Error
            ? error.message
            : "The photo observation could not be applied.",
          "error",
          4800,
        );
      }
  ...
  const renderEvidenceWorkspace = (
    ...
    <EvidenceWorkspacePanel
      key={`${initialTab}-${standalonePhotos ? "standalone" : "embedded"}`}
      ...
      photoConstraints={reconstruction.photoConstraints ?? []}
      ...
      activeReconstructionView={activeReconstructionView}
      physicsBakeState={
      ...
      onDeletePhoto={(photoId) =>
        setReconstruction((current) => ({
          ...
          photoConstraints: (current.photoConstraints ?? []).filter(
            (constraint) => constraint.photoId !== photoId,
          ),
          evidenceRecords: current.evidenceRecords.map((record) => ({
            ...record,
      ...
      onAddPhotoConstraint={addPhotoConstraint}
      onPhotoConstraintChange={updatePhotoConstraint}
      onApplyPhotoConstraint={handleApplyPhotoConstraint}
      onDeletePhotoConstraint={deletePhotoConstraint}
    />
  );
  ...
  return (
    <div
      className={`reconstruction-editor reconstruction-workspace reconstruction-workspace--${activeReconstructionView.toLowerCase()} ${
        activeTopWorkspaceTab === "photos"
          ? "reconstruction-workspace--photo-analysis"
          : ""
      }`}
      data-reconstruction-view={
        activeReconstructionView.toLowerCase()
      }
      data-workspace-tab={activeTopWorkspaceTab}
    >
          ...
          <div className="reconstruction-workspace__view-switch">
            {(["2D", "3D"] as const).map((view) => (
              <button
                ...
                onClick={() => {
                  setIsPlaying(false);
                  setActiveInvestigationDetail(null);
                  setActiveTopWorkspaceTab("scene");
                  setActiveReconstructionView(view);
                }}
                className={
                  activeReconstructionView === view
                    ? "is-active"
                    : ""
                }
              >
            ...
            <button
              ...
              onClick={() => {
                setIsPlaying(false);
                setActiveInvestigationDetail(null);
                setActiveTopWorkspaceTab((current) =>
                  current === "photos"
                    ? "scene"
                    : "photos",
                );
              ...
              aria-pressed={activeTopWorkspaceTab === "photos"}
              title="Toggle Photo-Assisted Reconstruction beside the main reconstruction"
            >
              <ImageIcon size={13} />
              Photo Assist
            </button>
          ...
          >
            <div className="reconstruction-workspace__legacy-scene-toolbar flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-[#494949] bg-[#292929] px-4 py-3">
              ...
              <div className="flex flex-wrap items-center gap-2 text-[10px] text-gray-600">
                ...
                <button
                  ...
                  onClick={() => {
                    setBezierEditMode(false);
                    setRouteDrawingParticipantId((current) =>
                      current
                        ? null
                        : selectedParticipantId,
                    );
                ...
                <button
                  ...
                  onClick={
                    handleToggleSelectedBezierSegment
                  }
                  className={`ui-button py-1.5 text-white disabled:opacity-40 ${
                    selectedBezierSegment?.enabled &&
                    bezierEditMode
                      ? "border-[#e8872d] bg-[#4a3323]"
                      : ""
                  }`}
                  title="Manual path only: switch the selected route segment to an investigator-controlled cubic Bezier curve."
                >
                  {selectedBezierSegment?.enabled
                    ? "Bezier: On"
                    : "Bezier Curve"}
                ...
                {selectedBezierSegment?.enabled &&
                  bezierEditMode && (
                    <button
                      type="button"
                      onClick={
                        handleResetSelectedBezierSegment
                      }
                      className="ui-button py-1.5"
                      title="Return the selected segment to RoadSafe automatic route smoothing."
                    >
                      Reset Curve
                    </button>
                  )}
            ...
            >
              ...
              {bezierEditMode &&
                selectedBezierSegment
                  ?.enabled && (
                  <div className="pointer-events-none absolute left-1/2 top-14 z-[95] -translate-x-1/2 border border-[#e8872d]/70 bg-[#252525]/95 px-4 py-2 text-[10px] font-bold text-[#f2f2f2] shadow-lg">
                    Manual Bezier · drag the two orange tangent handles · anchor points remain unchanged
                  </div>
                )}
            ...
            {!isPlaying &&
              ...
              {reconstruction.vehicles.map((participant, participantIndex) => {
                ...
                const vectorLength = Math.min(14, 3 + displaySpeedKmh / 8);
                const vectorRadians = (displayRotation * Math.PI) / 180;
                const vectorEnd = {
                  x: clamp(
                ...
                return (
                  <div key={participant.id}>
                    ...
                    {reconstruction.physicsSettings?.showVelocityVectors && (
                      ...
                      >
                        {displaySpeedKmh.toFixed(0)} km/h
                      </span>
                    )}
                      ...
                      selectedBezierSegment
                        ...
                        <>
                          ...
                          <button
                            type="button"
                            data-scene-interactive="true"
                            aria-label="Outgoing Bezier tangent handle"
                            title="Outgoing Bezier tangent — drag to shape the selected manual route segment"
                            onPointerDown={(event) => {
                              event.preventDefault();
                              event.stopPropagation();
                            ...
                            className="absolute z-[31] h-4 w-4 -translate-x-1/2 -translate-y-1/2 rotate-45 border-2 border-[#ffe0bd] bg-[#e8872d] shadow-[0_2px_8px_rgba(0,0,0,0.5)]"
                            style={{
                              left:
                                `${selectedBezierSegment.controlOne.x}%`,
                              top:
                                `${selectedBezierSegment.controlOne.y}%`,
                            }}
                          />
                          ...
                          <button
                            type="button"
                            data-scene-interactive="true"
                            aria-label="Incoming Bezier tangent handle"
                            title="Incoming Bezier tangent — drag to shape how the participant enters the segment endpoint"
                            onPointerDown={(event) => {
                              event.preventDefault();
                              event.stopPropagation();
                            ...
                            style={{
                              left:
                                `${selectedBezierSegment.controlTwo.x}%`,
                              top:
                                `${selectedBezierSegment.controlTwo.y}%`,
                            }}
                          />
                        </>
                      )}
                    ...
                    <button
                      ...
                      style={{
                        left: `${state.position.x}%`,
                        top: `${state.position.y}%`,
                        transform: `translate(-50%, -50%) translate(${shakeX}px, ${shakeY}px) rotate(${displayRotation + rotationShake}deg)`,
                      }}
                      title={`${participant.name} — ${displaySpeedKmh.toFixed(0)} km/h${reaction.emergencyBraking ? " · EMERGENCY BRAKING" : ""}`}
                    >
                      <ParticipantShape
                        participant={participant}
...
<section
          ...
          <details className="premium-investigation-card premium-investigation-card--physics" open>
            ...
            <div className="premium-investigation-card__body premium-physics-card">
              <div className="premium-physics-card__metrics">
                ...
                <div>
                  ...
                  <strong className="is-ready">
                    {rapierPhysicsRunning
                      ? "Running Rapier…"
                      : compactPhysicsSettings.enabled && reconstruction.vehicles.length > 0
                        ? reconstruction.lastPhysicsSimulation?.dynamicsEngine === "Rapier 3D Deterministic"
                          ? "Rapier baked"
                          : "Ready"
                        : "Needs setup"}
                  </strong>
              ...
              <button
                ...
                disabled={
                  rapierPhysicsRunning ||
                  !compactPhysicsSettings.enabled ||
                  reconstruction.vehicles.length === 0
                }
              >
                {rapierPhysicsRunning
                  ? "Running Rapier Dynamics…"
                  : "Run Rapier + Forensic Simulation"}
              </button>
            </div>
          </details>
        ...
        {activeTopWorkspaceTab === "photos" && (
          <aside
            className="reconstruction-photo-assist-dock"
            aria-label="Photo-Assisted Reconstruction integrated panel"
          >
            <header className="reconstruction-photo-assist-dock__header">
              <div className="min-w-0">
                <div className="flex min-w-0 items-center gap-2">
                  <ImageIcon size={13} />
                  <strong>Photo-Assisted Reconstruction</strong>
                </div>
                <span>
                  Linked to Main Reconstruction · {activeReconstructionView}
                </span>
              </div>

              <div className="reconstruction-photo-assist-dock__status">
                <span
                  className={
                    reconstruction.lastPhysicsSimulation
                      ? "is-baked"
                ...
                <button
                  ...
                  aria-label="Close Photo-Assisted Reconstruction"
                  title="Close Photo Assist"
                >
                  <X size={14} />
                </button>
              </div>
            ...
            <div className="reconstruction-photo-assist-dock__body">
              {renderEvidenceWorkspace("photos", true)}
            </div>
          </aside>
        )}
      </div>
