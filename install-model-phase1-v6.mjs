...
if (
  ...
  console.log(
    `Detected ${v1PartialFiles.size} leftover new model file(s) from an interrupted earlier install. V6 will reuse them safely.`,
  );
}
...
import {
  ...
  /*
   * If the shared asset-catalog import already existed from a partial pass,
   * make sure it also exposes the shared numeric colour resolver.
   */
  if (
    source.includes(
      'from "../../engine/assets/participantAssetCatalog";',
    ...
    source =
      source.replace(
        /import\s*\{\s*getParticipantPhysicalDimensions,\s*\}\s*from\s*["']\.\.\/\.\.\/engine\/assets\/participantAssetCatalog["'];?/,
`import {
  getParticipantColourNumber,
  getParticipantPhysicalDimensions,
} from "../../engine/assets/participantAssetCatalog";`,
  ...
  /*
   * The authored 3D path line used the old local PARTICIPANT_COLOURS map.
   * Move that visual to the same shared colour source as 2D/3D participant
   * bodies so model colour and path colour cannot diverge.
   */
  source =
    source.replace(
      /PARTICIPANT_COLOURS\s*\[\s*participant\.colour\s*\]\s*\?\?\s*0x[0-9a-fA-F]+/g,
...
`;
  ...
  /*
   * Catch malformed declaration joins before TypeScript/build.
   * V5's broad palette anchor left "const " immediately before
   * "function clamp", creating "const function clamp(...)".
   */
  for (
    const malformed of [
      "const function ",
    ...
    if (
      ...
      console.error(
        "Malformed TypeScript produced in Reconstruction3DViewer.tsx: " +
          malformed.trim(),
      );
      restoreAll();
      process.exit(1);
...
try {
  ...
  console.error(
    "\nModel Phase 1 V6 failed verification/build. All changes were restored.",
  );
...
console.log(`
RoadSafe Model Phase 1 V6 installed successfully.
