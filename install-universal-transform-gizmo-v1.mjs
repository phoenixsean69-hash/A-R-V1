...
const buildLogPath =
  path.join(
    backupRoot,
    "universal-transform-gizmo-v1-build.log",
  );
...
`  const realSceneGeometry = settings.realSceneGeometry?.status === "ready"
  ...
  if (
    ...
    fail(
      "Could not locate RoadSceneEnvironment realSceneGeometry declaration. No files changed.",
    );
  }
...
`export default function RealSceneGeometryLayer({
  ...
  if (
    ...
    fail(
      "Could not locate RealSceneGeometryLayer signature. No files changed.",
    );
  }
...
const output =
  [
    "RoadSafe Universal Transform Gizmo V1",
    "=====================================",
    `status: ${String(
      build.status,
    )}`,
...
console.log("");
console.log(
  "RoadSafe Universal Transform Gizmo V1 installed successfully.",
);
...
console.log(
  "  node revoke-universal-transform-gizmo-v1.mjs",
);
