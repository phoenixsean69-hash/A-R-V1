...
function createLegacyApproachPoints(
  participant: ReconstructionVehicle,
...
function normaliseParticipant(
  ...
  return syncLegacyParticipantFields({
    ...
    finalRotation: Number(
      participant.finalRotation ??
        impactPoint.rotation,
    ),
    collisionTimeSeconds:
      impactPoint.timeSeconds,
    estimatedSpeedKmh: Math.max(
      0,
      Number(
        participant
          .estimatedSpeedKmh ??
...
function normaliseSceneObject(
  ...
  return {
    ...
    traceSmoothing: clamp(
      Number(
        object.traceSmoothing ??
          0.85,
      ),
      0,
      1,
...
function normalisePhoto(
  ...
  return {
    ...
    bearingDegrees: Number(
      photo.bearingDegrees ?? 0,
    ),
  };
}

function normalisePhotoConstraint(
  ...
  return {
    ...
    headingDegrees:
      constraint.headingDegrees === undefined
        ? undefined
        : ((Number(constraint.headingDegrees) % 360) + 360) % 360,
    notes:
      constraint.notes ?? "",
    createdAt:
...
function normaliseFieldCalibration(
  ...
  return {
    ...
    directionReferenceDistanceMetres:
      Math.max(
        0,
        Number(
          calibration
...
function normaliseFieldPlacement(
  ...
  return {
    ...
    sampleCount: Math.max(
      1,
      Number(
        placement.sampleCount ?? 1,
      ),
    ),
    averageAccuracyMetres:
      Math.max(
        0,
        Number(
          placement
    ...
    rawSamples:
      Array.isArray(
        placement.rawSamples,
      )
        ? placement.rawSamples.map(
            normaliseGeoCoordinate,
          )
        : undefined,
    rejectedSamples:
      Array.isArray(
        placement.rejectedSamples,
      )
        ? placement.rejectedSamples.map(
            (sample) => ({
              coordinate:
                normaliseGeoCoordinate(
                  sample.coordinate,
...
function normaliseFieldWalkingTrack(
  ...
  return {
    ...
    id:
      track.id ||
      createId("field-track"),
    captureMode:
      track.captureMode ?? "Line",
    coordinates:
      Array.isArray(
        track.coordinates,
      )
    ...
    rawCoordinates:
      Array.isArray(
        track.rawCoordinates,
      )
        ? track.rawCoordinates.map(
            normaliseGeoCoordinate,
          )
        : undefined,
    rejectedCoordinates:
      Array.isArray(
        track.rejectedCoordinates,
      )
        ? track.rejectedCoordinates.map(
            (sample) => ({
              coordinate:
                normaliseGeoCoordinate(
                  sample.coordinate,
    ...
    rawScenePoints:
      Array.isArray(
        track.rawScenePoints,
      )
        ? track.rawScenePoints.map(
            (point) => ({
              x: Number(
                point.x ?? 50,
              ),
    ...
    distanceMetres: Math.max(
      0,
      Number(
        track.distanceMetres ?? 0,
      ),
    ),
    rawDistanceMetres:
      track.rawDistanceMetres ===
      undefined
        ? undefined
        : Math.max(
            0,
    ...
    processingMethod:
      track.processingMethod ??
      "Legacy walking trace",
    recordedBy:
      track.recordedBy ?? "",
  };
}
...
function normaliseCollisionSetup(
  ...
  return {
    ...
    toleranceMetres: Math.max(
      0.2,
      Number(
        setup?.toleranceMetres ?? 2,
      ),
    ),
    notes: setup?.notes ?? "",
    lastCalculatedAt:
      setup?.lastCalculatedAt,
  };
}
...
function isReconstructionRecord(
  ...
  return Boolean(
    value &&
      ...
      !Array.isArray(value) &&
      typeof (
        value as Partial<AccidentReconstruction>
      ).id === "string",
  );
}
  ...
  return {
    ...
    timelineEvents:
      Array.isArray(
        ...
        ? reconstruction.timelineEvents.map(
            (event) =>
    ...
    photos:
      Array.isArray(
        ...
        ? reconstruction.photos.map(
            normalisePhoto,
          )
        : [],
    photoConstraints:
      Array.isArray(
        reconstruction.photoConstraints,
      )
        ? reconstruction.photoConstraints.map(
            normalisePhotoConstraint,
          )
        : [],
    siteCoordinate:
    ...
    fieldWalkingTracks:
      Array.isArray(
        ...
        ? reconstruction
            .fieldWalkingTracks.map(
              normaliseFieldWalkingTrack,
            )
        : [],
    collisionSetup:
      normaliseCollisionSetup(
        reconstruction
          .collisionSetup,
      ),
    physicsSettings:
      normalisePhysicsSettings(
        reconstruction
          .physicsSettings,
      ),
...
function readLocalReconstructions(): AccidentReconstruction[] {
  try {
    ...
    const parsedValue =
      JSON.parse(
        storedValue,
      ) as AccidentReconstruction[];

    if (!Array.isArray(parsedValue)) {
      return [];
    }

    return parsedValue.flatMap(
      (record, index) => {
        if (
          !isReconstructionRecord(
            record,
          )
        ) {
          console.warn(
    ...
    console.error(
      "Failed to read reconstructions:",
      error,
    );
...
export const ReconstructionService = {
  ...
  delete(
    ...
    const remainingRecords =
      readLocalReconstructions().filter(
        (record) =>
          record.id !==
          reconstructionId,
      );
