...
export default function ForensicInvestigationWorkspace({
  ...
  const addVehicleDamagePhotos = async (
    ...
    try {
      ...
      setMessage(
        `${stored.length} damage photograph${stored.length === 1 ? "" : "s"} attached.`,
      );
    } catch (error) {
      console.error(
              ...
              <section className="fv2-panel">
                ...
                <div className="fv2-grid">
                    ...
                    <div className="fv2-check-grid">
                      {VEHICLE_DAMAGE_AREA_OPTIONS.map((area) => (
                        <label key={area}>
                          <input
                            type="checkbox"
                            checked={vehicleDamageAreas.has(area)}
                  ...
                  <div className="fv2-field full fv2-damage-photo-section">
                    <span>Damage photographs</span>

                    <div className="fv2-damage-photo-upload">
                      <label className="fv2-photo-picker">
                        <input
                          type="file"
                          accept="image/*"
                          multiple
                        ...
                        <span>
                          {vehicleDamagePhotoBusy
                            ? "Saving photos..."
                            : "Choose damage photos"}
                        </span>
                      </label>

                      <small>
                        Upload clear photographs of the damaged areas from useful angles. Up to 12 photos, 15 MB each. For this prototype, the image files are stored locally on this device.
                      </small>
                    </div>

                    {vehicleDamagePhotos.length > 0 && (
                        ...
                        {investigation.vehicles.map((record) => (
                          <tr key={record.id}>
                            ...
                            <td>
                              ...
                              <small>{record.damageAreas.length ? record.damageAreas.join(", ") : "No areas selected"}</small>
                              <small>
