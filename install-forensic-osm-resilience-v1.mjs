...
const statePath =
  path.join(
    backupRoot,
    "last-forensic-osm-resilience-v1.json",
  );

const buildLogPath =
  path.join(
    backupRoot,
    "forensic-osm-resilience-v1-build.log",
  );
...
`const OVERPASS_ENDPOINTS = Array.from(
  new Set(
    [
      ...
      // private.coffee replaced the old overpass.kumi.systems hostname.
...
if (
  ...
  fail(
    "Deprecated overpass.kumi.systems endpoint survived the patch. No files changed.",
  );
}
...
const output =
  [
    "RoadSafe Forensic OSM Resilience V1",
    "===================================",
    `status: ${String(
      build.status,
    )}`,
...
console.log("");
console.log(
  "RoadSafe Forensic OSM Resilience V1 installed successfully.",
);
...
console.log(
  "  node revoke-forensic-osm-resilience-v1.mjs",
);
