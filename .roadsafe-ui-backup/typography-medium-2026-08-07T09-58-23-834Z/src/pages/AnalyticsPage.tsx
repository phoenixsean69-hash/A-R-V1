      ...
      <section className="grid gap-3 xl:grid-cols-[1.35fr_.85fr]">
        <ChartPanel title="Accident and case activity" subtitle="Monthly comparison across real stored cases and accident records">
          <Line
            ...
            options={{ responsive: true, maintainAspectRatio: false, plugins: commonPlugins, scales: { x: { ticks: { color: chartText, font: { size: 12 } }, grid: { color: chartGrid } }, y: { beginAtZero: true, ticks: { color: chartText, precision: 0, font: { size: 12 } }, grid: { color: chartGrid } } } }}
          />
        </ChartPanel>
      ...
      <section className="grid gap-3 xl:grid-cols-2">
        <ChartPanel title="Leading collision causes" subtitle="Ranked from the recorded accident dataset">
          <Bar
            ...
            options={{ indexAxis: "y", responsive: true, maintainAspectRatio: false, plugins: { ...commonPlugins, legend: { display: false } }, scales: { x: { beginAtZero: true, ticks: { color: chartText, precision: 0, font: { size: 12 } }, grid: { color: chartGrid } }, y: { ticks: { color: chartText, font: { size: 12 } }, grid: { display: false } } } }}
          />
        </ChartPanel>
        ...
        <ChartPanel title="Reconstruction evidence readiness" subtitle="Average scene documentation depth">
          <Radar
            ...
            options={{ responsive: true, maintainAspectRatio: false, plugins: commonPlugins, scales: { r: { min: 0, max: 100, ticks: { display: false }, pointLabels: { color: chartText, font: { size: 12 } }, angleLines: { color: chartGrid }, grid: { color: chartGrid } } } }}
          />
        </ChartPanel>
      </section>
        ...
        <ChartPanel title="Participant types" subtitle="Road users represented across saved reconstructions">
          {participants.length ? (
            <Bar
              ...
              options={{ responsive: true, maintainAspectRatio: false, plugins: { ...commonPlugins, legend: { display: false } }, scales: { x: { ticks: { color: chartText, font: { size: 12 } }, grid: { display: false } }, y: { beginAtZero: true, ticks: { color: chartText, precision: 0, font: { size: 12 } }, grid: { color: chartGrid } } } }}
            />
