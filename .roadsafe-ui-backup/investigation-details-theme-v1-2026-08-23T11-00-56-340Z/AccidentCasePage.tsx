...
export default function AccidentCasePage() {
  ...
  return (
    <div className="case-workspace min-w-0 space-y-3">
      <div className="mx-auto min-w-0 max-w-[1600px] space-y-3">
        <header className="ui-panel flex flex-wrap items-center justify-between gap-4 px-4 py-3">
          <div className="min-w-0">
            ...
            <h1 className="mt-1 truncate text-lg font-bold text-slate-100">
              {record.title}
            </h1>
            ...
            <p className="mt-1 text-[9px] font-semibold text-slate-600">
              Reconstruction last saved:{" "}
              {formatSavedAt(stats.reconstructionLastSavedAt)}
            </p>
          </div>

          <div className="flex flex-wrap gap-2">
            <Link to="/cases" className="ui-button">
              ← Cases
            </Link>
            <Link to={`/cases/${record.id}/edit`} className="ui-button">
              Edit Case
            </Link>
            <Link
            ...
            <Link to={`/cases/${record.id}/report`} className="ui-button">
              Generate Report
            </Link>
          </div>
        </header>
        ...
        <section className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5 xl:grid-cols-9">
          {statCards.map(([label, value]) => (
            <div key={label} className="ui-panel min-w-0 p-3">
              <p className="break-words text-lg font-bold text-slate-100">
                {value}
              </p>
              <p className="mt-1 text-[9px] font-semibold text-slate-600">
                {label}
        ...
        <div className="grid min-w-0 items-start gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(300px,360px)]">
          <main className="min-w-0 space-y-3">
            <section className="ui-panel min-w-0 p-4">
              <h2 className="ui-panel-title">Case summary</h2>
              <p className="mt-3 whitespace-pre-wrap break-words text-[11px] leading-6 text-slate-400">
            ...
            <section className="ui-panel min-w-0 p-4">
              ...
              <dl className="mt-4 min-w-0 space-y-3 text-[10px]">
                ...
                <div className="min-w-0">
                  ...
                  <dd className="mt-0.5 break-all text-[10px] text-slate-300">
            ...
            <section className="ui-panel min-w-0 p-4">
              <label className="block text-[10px] font-bold text-slate-400">
                ...
                <select
                  ...
                  onChange={(event) => {
                    ...
                    if (result.blocked) {
                      setStatusMessage(
                        result.message ?? "Status update blocked.",
                      );
                  ...
                  className="ui-input mt-2 w-full min-w-0"
                >
              ...
              {!completion.complete && (
                <p className="mt-3 text-[9px] leading-5 text-[#d9bd78]">
                  Reconstruction Complete remains blocked until all checklist
                  items pass.
                </p>
              )}
