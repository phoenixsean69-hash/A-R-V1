...
export default function ForensicDatumPicker({
  ...
  const datumMarkerRef =
    useRef<maplibregl.Marker | null>(
      null,
    );

  const anchorMarkerRef =
    useRef<maplibregl.Marker | null>(
      null,
    );
  ...
  const [message, setMessage] =
    useState(
      currentDatum
        ? "Existing datum loaded. Click inside the frozen core to move it."
        : "Click a permanent feature inside the frozen forensic core.",
    );
  ...
  const geometry =
    ...
    accidentCase
      .roadLayoutDetection
      ?.suggestedSceneSettings
      ?.realSceneGeometry;

  const selection =
    geometry?.selection ?? null;

  const accidentAnchor:
  ...
  useEffect(() => {
    ...
    const handleError = (
      ...
      console.error(
        "Forensic datum picker map error:",
        event.error,
      );
    ...
    const handleClick = (
      ...
      if (
        ...
        setMessage(
          "Reference point rejected: click inside the orange frozen forensic core.",
        );
  ...
  if (
    ...
    return createPortal(
      <div className="fixed inset-0 z-[9999] grid place-items-center bg-black/85 p-4">
        <div className="w-full max-w-lg rounded-md border border-[#494949] bg-[#202020] p-5 shadow-2xl">
          ...
          <p className="mt-3 text-[10px] leading-5 text-slate-400">
            RoadSafe could not read the frozen
            forensic core from the linked
            reconstruction.
          </p>
  ...
  const confirm = () => {
    if (
      ...
      setMessage(
        "Click the map to select the fixed reference point first.",
      );
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex bg-black/90 p-2 sm:p-4">
      <section className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden rounded-md border border-[#494949] bg-[#202020] shadow-[0_30px_100px_rgba(0,0,0,.78)]">
        <header className="flex shrink-0 items-start justify-between gap-4 border-b border-[#494949] bg-[#303030] px-4 py-3">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-[9px] text-slate-500">
              Orange = frozen forensic core.
              Red = accident anchor.
              Click a permanent feature inside
              the core.
            </p>
          </div>
        ...
        <div className="grid min-h-0 flex-1 gap-3 p-3 xl:grid-cols-[minmax(0,1fr)_340px]">
          <div className="relative min-h-[420px] min-w-0 overflow-hidden rounded-md border border-[#494949] bg-[#151515]">
            <div
              ref={containerRef}
            ...
            {mapStatus ===
              "loading" && (
              <div className="pointer-events-none absolute inset-0 z-10 grid place-items-center bg-[#151515]">
                <div className="rounded border border-[#494949] bg-[#202020] px-4 py-3 text-[9px] text-slate-400">
                  Loading frozen scene map…
                </div>
              </div>
            )}

            {mapError && (
              <div className="pointer-events-none absolute left-3 right-3 top-3 z-20 rounded border border-[#713646] bg-[#321722]/95 px-3 py-2 text-[8px] text-[#f09aae]">
                Map source warning:{" "}
                {mapError}
              </div>
            )}

            <div className="pointer-events-none absolute bottom-3 left-3 z-20 flex flex-wrap gap-2 rounded border border-[#494949] bg-[#202020]/90 px-2.5 py-2 text-[8px] text-slate-400">
              <span>
          ...
          <aside className="min-h-0 overflow-y-auto rounded-md border border-[#494949] bg-[#292929]">
            ...
            <div className="space-y-3 p-3">
              ...
              <div className="rounded border border-[#414141] bg-[#202020] p-3">
                ...
                <p className="mt-2 break-all font-mono text-[9px] text-slate-300">
                  {picked
                    ? `${picked.latitude.toFixed(
                        7,
                      )}, ${picked.longitude.toFixed(
                        7,
                      )}`
                    : "Not selected"}
                </p>
              ...
              <div className="rounded border border-[#6d5523] bg-[#241d10] p-3 text-[8px] leading-4 text-[#c6ad73]">
                ...
                utility-pole base, signpost
                base, drain corner, culvert
                corner, surveyed monument,
                or another stable point.
              </div>
