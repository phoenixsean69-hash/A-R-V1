        ...
        <ReportSection title="Field GPS Placement Audit">
          ...
          <RecordList
            ...
            items={reconstruction?.fieldPlacements.map(
              (placement) => `${placement.targetLabel}: ${placement.coordinate.latitude.toFixed(7)}, ${placement.coordinate.longitude.toFixed(7)}; accuracy ±${placement.averageAccuracyMetres.toFixed(1)}m; ${placement.method}; ${placement.sampleCount} sample(s)${placement.acceptedPoorAccuracy ? "; poor accuracy explicitly accepted" : ""}${placement.manuallyAdjusted ? "; manually adjusted afterward" : ""}.`,
            ) ?? []}
          />
        </ReportSection>

        <ReportSection title="Field Walking Traces">
          <RecordList
            empty="No walking traces recorded."
            items={reconstruction?.fieldWalkingTracks.map(
              (track) => `${track.targetLabel}: ${track.coordinates.length} GPS points, ${track.distanceMetres.toFixed(2)} metres, average accuracy ±${track.averageAccuracyMetres.toFixed(1)}m.`,
            ) ?? []}
          />
        </ReportSection>
