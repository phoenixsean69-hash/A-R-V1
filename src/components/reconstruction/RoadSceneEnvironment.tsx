...
function groundTexture(settings: RoadSceneSettings): string {
  const base = settings.timeOfDay === "Night" ? "#131a20" : "#273129";
  return [
    "radial-gradient(circle at 15% 30%, rgba(109,132,106,.28) 0 1px, transparent 1.5px)",
    "radial-gradient(circle at 72% 68%, rgba(0,0,0,.18) 0 1px, transparent 1.5px)",
    `linear-gradient(${base}, ${base})`,
  ].join(",");
}
...
export default function RoadSceneEnvironment({ settings }: RoadSceneEnvironmentProps) {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden" style={{ background: groundTexture(settings), backgroundSize: "9px 9px,12px 12px,auto" }}>
      <div className="absolute inset-0 opacity-30" style={{ backgroundImage: "linear-gradient(90deg,rgba(255,255,255,.025) 1px,transparent 1px),linear-gradient(rgba(255,255,255,.025) 1px,transparent 1px)", backgroundSize: "40px 40px" }} />
      <div className="absolute inset-0 origin-center" style={{ transform: `rotate(${settings.roadRotation}deg) scale(1.08)` }}>
        {settings.roadLayout === "Four-way Intersection" && <><RoadStrip orientation="horizontal" width={roadWidth} settings={settings} /><RoadStrip orientation="vertical" width={roadWidth} settings={settings} /></>}
        {settings.roadLayout === "T-Junction" && <><RoadStrip orientation="horizontal" width={roadWidth} settings={settings} /><RoadStrip orientation="vertical" width={roadWidth} settings={settings} start={50} length={50} /></>}
        {settings.roadLayout === "Straight Road" && <RoadStrip orientation="horizontal" width={roadWidth} settings={settings} />}
        {settings.roadLayout === "Pedestrian Crossing" && <><RoadStrip orientation="horizontal" width={roadWidth} settings={settings} /><ZebraCrossing orientation="vertical" /></>}
        {settings.roadLayout === "Roundabout" && <Roundabout width={roadWidth} settings={settings} />}
        {settings.roadLayout === "Transport Terminus" && <TransportTerminus width={roadWidth} settings={settings} />}
        {settings.showPedestrianCrossing && settings.roadLayout !== "Pedestrian Crossing" && <ZebraCrossing orientation="vertical" />}
        ...
        {settings.roadSurface === "Damaged" && (
          <div className="absolute inset-0 z-[3] opacity-55">
            {[
              ...
              ["70%", "54%", "70px", "-7deg"],
              ["49%", "21%", "54px", "86deg"],
            ].map(([left, top, width, rotate], index) => <span key={index} className="absolute h-[2px] bg-black/80" style={{ left, top, width, transform: `rotate(${rotate})` }} />)}
          </div>
        )}
      </div>
      {settings.roadSurface === "Wet" && <div className="absolute inset-0 z-[2] bg-gradient-to-br from-sky-100/5 via-transparent to-sky-200/10 mix-blend-screen" />}
      <div className="absolute bottom-3 left-3 z-20 rounded-md border border-white/10 bg-[#05080e]/75 px-3 py-2 text-[9px] font-semibold text-slate-200 backdrop-blur-sm">
        {settings.roadLayout} · {settings.laneCount} lane{settings.laneCount === 1 ? "" : "s"} · {settings.roadSurface}
      </div>
      <div className="absolute right-3 top-3 z-20 grid h-12 w-12 place-items-center rounded-full border-[3px] border-[#8f2830] bg-[#e8e9e6] text-center shadow-lg">
        <span className="text-sm font-black leading-none text-[#17191b]">{settings.speedLimitKmh}<span className="block text-[5px] uppercase tracking-wide">km/h</span></span>
      </div>
