...
export default function PhotoConstraintWorkspace({
  ...
  const syncTargetToMainReconstruction = (
    constraint: Pick<
      ScenePhotoConstraint,
      | "kind"
      | "participantId"
  ...
  const editConstraint = (
    ...
    syncTargetToMainReconstruction(
      constraint,
    );
  };
  ...
  return (
    <section className="mt-5 overflow-hidden rounded-xl border border-[#494949] bg-[#242424] text-slate-200 shadow-sm">
      ...
      <div className="border-b border-[#3d3d3d] bg-[#202020] px-4 py-2 text-[8px] text-slate-500">
        Main Reconstruction selection · {
          selectedParticipantId
            ? participants.find((item) => item.id === selectedParticipantId)?.name ?? "Participant"
            : selectedSceneObjectId
              ? sceneObjects.find((item) => item.id === selectedSceneObjectId)?.label ?? "Scene object"
              : "No participant/object selected"
        }
        {selectedPathPointId && selectedParticipantId
          ? ` · ${
              participants
                .find((item) => item.id === selectedParticipantId)
                ?.pathPoints.find((point) => point.id === selectedPathPointId)
                ?.label ?? "Path point"
            }`
          : ""}
      </div>

      <div className="grid gap-3 p-3 xl:grid-cols-[minmax(0,1.25fr)_minmax(280px,0.75fr)]">
