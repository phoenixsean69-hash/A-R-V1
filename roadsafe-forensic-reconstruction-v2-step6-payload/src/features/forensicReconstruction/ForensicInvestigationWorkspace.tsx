              ...
              <section className="fv2-panel">
                <header>
                  <div>
                    ...
                    <strong>Automatic consistency scan</strong>
                  </div>
                  <div className="fv2-analysis-summary">
                ...
                <div className="fv2-analysis-rule">
                  This scan only highlights missing, conflicting or unresolved inputs already recorded in RoadSafe. It does not decide fault, invent a crash sequence or replace investigator judgment.
                </div>
              ...
              <section className="fv2-panel">
                <header>
                  <div>
                    ...
                    <strong>Record an evidence-based analytical finding</strong>
                  </div>
                </header>
                ...
                <div className="fv2-grid">
                  ...
                  <label className="fv2-field full">
                    <span>Finding / analytical conclusion</span>
                    <textarea
                      rows={5}
                      value={analysisFinding}
                      onChange={(e) => setAnalysisFinding(e.target.value)}
                      placeholder="State what the current evidence supports, contradicts, or leaves unresolved. Do not assign legal guilt here."
                    />
                  </label>
                  ...
                  <div className="fv2-field full">
                    <span>Supporting physical evidence</span>
                    {investigation.evidence.length === 0 ? (
                  ...
                  <div className="fv2-field full">
                    <span>Supporting measurements / calculations</span>
                    {investigation.measurements.length === 0 ? (
                  ...
                  <div className="fv2-field full">
                    <span>Vehicle examination sources</span>
                    {investigation.vehicles.length === 0 ? (
                  ...
                  <div className="fv2-field full">
                    <span>Person / driver sources</span>
                    {investigation.persons.length === 0 ? (
                  ...
                  <div className="fv2-field full">
                    <span>Witness statement sources</span>
                    {investigation.witnesses.length === 0 ? (
                ...
                <footer><button className="primary" onClick={addAnalysisFinding}>Add analysis finding</button></footer>
              </section>

              <section className="fv2-panel">
                <header><span>Analysis register</span><strong>{investigation.analysisFindings.length} finding(s)</strong></header>
                {investigation.analysisFindings.length === 0 ? (
                  <div className="fv2-empty">No analytical findings recorded yet.</div>
                ) : (
                  <div className="fv2-tablewrap">
                    <table>
                      <thead><tr><th>ID</th><th>Category / method</th><th>Finding</th><th>Status</th><th>Source basis</th><th>Limitations</th><th>Origin / confidence</th><th /></tr></thead>
                      <tbody>
                        {investigation.analysisFindings.map((finding) => {
                          ...
                          return (
                            <tr key={finding.id}>
                              <td><b>{finding.code}</b></td>
                              <td>{finding.category}<small>{finding.method}</small></td>
                              <td>{finding.finding}<small>{finding.followUpAction || "No follow-up recorded"}</small></td>
                              <td><span className={`tag ${finding.status === "Conflicting evidence" ? "derived" : ""}`}>{finding.status}</span></td>
                              <td>{sourceCount} source{sourceCount === 1 ? "" : "s"}<small>E {finding.sourceEvidenceIds.length} · M {finding.sourceMeasurementIds.length} · V {finding.sourceVehicleIds.length} · P {finding.sourcePersonIds.length} · W {finding.sourceWitnessIds.length}</small></td>
                              <td>{finding.limitations.length ? finding.limitations.join(", ") : "None recorded"}<small>{finding.limitationNotes || "—"}</small></td>
                              <td>{finding.origin}<small>{finding.confidence}</small></td>
                              <td><button className="danger" onClick={() => setInvestigation(ForensicInvestigationService.deleteAnalysisFinding(investigation, finding.id))}>Remove</button></td>
                            </tr>
                          );
                        })}
                      </tbody>
