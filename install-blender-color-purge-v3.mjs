...
const statePath = path.join(
  root,
  ".roadsafe-ui-backup",
  "last-blender-color-purge-v3.json",
);
...
function copyPayload(payloadRelativePath, destinationRelativePath) {
  const sourcePath = path.join(root, payloadRelativePath);

  if (!fs.existsSync(sourcePath)) {
    throw new Error(`Missing payload file: ${payloadRelativePath}`);
  }
...
/*
 * CSS can contain literal selectors such as:
 *   [class*="bg-blue-600"]
 * or escaped utility selectors:
 *   .bg-sky-500
 *
 * These rules only existed to compensate for the old theme. Remove entire
 * CSS rules whose selector itself references a legacy cool utility.
 */
const cssRuleWithCoolSelector =
  /([^{}]*?(?:bg|text|border|ring|outline|divide|fill|stroke|accent|from|via|to)-(?:blue|indigo|sky|cyan|purple|violet)-\d{2,3}[^{}]*?)\{[^{}]*\}/gi;
...
for (const absolutePath of walk(srcRoot)) {
  ...
  if (relativePath.endsWith(".css")) {
    ...
    next = next.replace(
      cssRuleWithCoolSelector,
      "\n/* Legacy cool-colour compensation rule removed by Blender Color Purge V3. */\n",
    );
...
try {
  ...
  console.error(
    "\nBlender Color Purge V3 failed verification/build. All changes were restored.",
  );
...
console.log(`
Blender Color Purge V3 completed successfully.

V3 specifically fixes:
- cool-colour class names embedded inside CSS selectors;
- .ts service/renderer files missed by V2;
- #cbd5e1 in report/canvas renderer source;
- the exact 12 traces reported by the V2 audit.
...
Rollback:
  node revoke-blender-color-purge-v3.mjs
`);
