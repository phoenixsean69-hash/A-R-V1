...
function restore() {
  ...
  console.log("Restored pre-V6 files.");
}
...
/*
 * Verify the old duplicated controls are gone once the real panel exists.
 */
if (
  ar.includes(
    "roadsafe-ar-blender-properties",
  ...
  const oldHeadingIndex =
    findRegexIndex(
      ar,
      /\{\s*calibrationStage\s*===\s*"heading"\s*&&\s*\(/m,
    );

  const oldPlaybackIndex =
    findRegexIndex(
      ar,
      /\{\s*playbackReady\s*&&\s*\(/m,
    );

  if (oldHeadingIndex >= 0) {
    fail(
      "Old AR heading overlay still exists after migration.",
    );
  }

  if (oldPlaybackIndex >= 0) {
    fail(
      "Old AR playback overlay still exists after migration.",
    );
  }
}
...
console.log("");
console.log(
  "RoadSafe Blender Properties 2D + AR V6 installed.",
);
console.log("");
console.log("2D:");
...
console.log(
  "  node revoke-blender-properties-2d-ar-v6.mjs",
);
