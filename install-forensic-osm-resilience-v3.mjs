...
const statePath =
  path.join(
    backupRoot,
    "last-forensic-osm-resilience-v3.json",
  );

const buildLogPath =
  path.join(
    backupRoot,
    "forensic-osm-resilience-v3-build.log",
  );
...
/* ========================================================================== */
/* 0. Strengthen forensic Overpass response typing.                           */
/* ========================================================================== */

/*
 * Phase 1 intentionally started with `elements?: unknown[]` because the raw
 * payload was only archived. The resilience patch now also seeds that exact
 * payload into RealSceneExtractionService, whose contract is:
 *
 *   elements?: OverpassElement[]
 *
 * Keep the forensic service structurally compatible with that contract.
 */
const weakOverpassRaw =
...
if (
  ...
  pipeline =
    replaceOnce(
      ...
      weakOverpassRaw,
      strongOverpassRaw,
      "weak forensic OverpassRaw interface",
    );
} else if (
  !pipeline.includes(
    'interface OverpassElement {',
  ...
  fail(
    "Could not establish the forensic Overpass element contract. No files changed.",
  );
}

/*
 * Runtime guard: external JSON remains untrusted even though our TypeScript
 * contract is strong. Validate the fields the geometry normalizer relies on
 * before the payload is archived/seeded.
 */
const validationInsertionPoint =
  pipeline.indexOf(
    "const OVERPASS_ENDPOINTS =",
  );

if (
  validationInsertionPoint < 0
) {
  fail(
    "Could not locate Overpass endpoint declaration for payload validation helper. No files changed.",
  );
}
...
console.log(
  "PATCHED forensic Overpass payload contract: typed + runtime validated.",
);
...
if (
  ...
  fail(
    "Weak OverpassRaw elements typing survived the V3 patch. No files changed.",
  );
}

console.log(
  "Forensic -> extraction Overpass contract audit: PASS",
);
...
const output =
  [
    "RoadSafe Forensic OSM Resilience V3",
    "===================================",
    `status: ${String(
      build.status,
    )}`,
...
console.log("");
console.log(
  "RoadSafe Forensic OSM Resilience V3 installed successfully.",
);
...
console.log(
  "  node revoke-forensic-osm-resilience-v3.mjs",
);
