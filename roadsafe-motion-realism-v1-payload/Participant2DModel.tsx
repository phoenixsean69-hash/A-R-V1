      ...
      {brakingIntensity > 0.02 && (
        <>
          <rect
            x={length * 0.065}
            y={width * 0.16}
            width={Math.max(0.035, length * 0.032)}
            height={width * 0.22}
            rx={width * 0.025}
            fill="#ff3b30"
            opacity={0.6 + brakingIntensity * 0.4}
          />
          <rect
            x={length * 0.065}
            y={width * 0.62}
...
function HumanArtwork({
  ...
  const cadenceHz =
    1.35 +
    Math.min(1, speedRatio) * 0.7 +
    Math.max(0, speedRatio - 1) * 0.55;
  ...
  return (
    <>
      ...
      <line
        x1={shoulderX}
        y1={width * 0.13}
        x2={length * 0.36 + armTravel}
        y2={width * 0.03 - shoulderSpread}
        stroke="#b97850"
        strokeWidth={0.055}
        strokeLinecap="round"
      />
  ...
  if (asset.family === "Human") {
    return (
      <HumanArtwork
        ...
        motionTimeSeconds={motionTimeSeconds}
        motionSpeedKmh={motionSpeedKmh}
