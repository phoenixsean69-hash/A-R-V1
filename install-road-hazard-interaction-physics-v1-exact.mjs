...
const PAYLOAD = path.join(
  INSTALLER_DIR,
  "roadsafe-road-hazard-interaction-physics-v1-payload",
);
...
for (const [rel, expectedHash] of Object.entries(EXPECTED)) {
  ...
  if (currentHash !== expectedHash) {
    fail([
      `${rel} differs from the exact fresh local physics bundle used for Road Hazard Interaction Physics V1.`,
...
const backupDir = path.join(
  backupRoot,
  `road-hazard-interaction-physics-v1-${stamp}`,
);
...
console.log("");
console.log("RoadSafe Road Hazard Interaction Physics V1 — EXACT LOCAL");
console.log("=========================================================");
...
if (verify.error || verify.status !== 0) {
  ...
  console.error("");
  console.error("[RoadSafe] Rolling Road Hazard Interaction Physics V1 back automatically...");
  rollback();
...
console.log("");
console.log("Road Hazard Interaction Physics V1 is installed.");
