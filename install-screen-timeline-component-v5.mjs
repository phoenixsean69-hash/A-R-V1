...
const statePath =
  path.join(
    backupRoot,
    "last-screen-timeline-component-v5.json",
  );

const buildLogPath =
  path.join(
    backupRoot,
    "screen-timeline-component-v5-build.log",
  );
...
function restoreV4() {
  ...
  console.log(
    "RESTORED clean pre-V5 timeline state.",
  );
}
...
if (
  ...
  if (
    ...
    if (!editor.includes(nodeImport)) {
      fail(
        "Could not locate reconstruction component import area. No V4 files written.",
      );
    }
...
if (
  ...
  fail(
    "Could not isolate the old inline playback/timeline block. No V4 files written.",
  );
}
...
if (
  ...
  fail(
    "The isolated block did not contain the expected old Timeline. No V4 files written.",
  );
}
...
`if (tool === "Timeline") {
  ...
  fail(
    "Could not locate Timeline toolbar behavior. No V4 files written.",
  );
}
...
if (
  ...
  if (rootCloseIndex < 0) {
    fail(
      "Could not locate reconstruction root close. No V4 files written.",
    );
  }
...
try {
  ...
  console.log(
    "V5 editor + component TSX parse audit: PASS",
  );
} catch (error) {
  if (
...
function runBuild() {
   ...
   *
   * npm.cmd + shell:false can fail at process-launch level on Windows and
   * return:
   *   status: null
   *   stdout: ""
   *   stderr: ""
   *   error: <spawn error>
   *
   * That is exactly why V4 produced an empty build log.
   *
   * V5 executes npm through cmd.exe on Windows, which is the same command
   * path the user's PowerShell ultimately resolves successfully.
   */
  const command =
  ...
  const diagnosticHeader = [
    "RoadSafe Screen Timeline Component V5 build runner",
    "==================================================",
  ...
  if (
    ...
    console.error(
      "Build process returned status=null without a launch error.",
    );
  }
...
/*
 * Extracting playback into its own component can leave icon imports in
 * AccidentReconstructionEditor that TypeScript flags as TS6133.
 *
 * Clean only imports explicitly named by TS6133, and only if they occur in the
 * materialIcons import block and nowhere else in the editor.
 */
function repairUnusedEditorIconImports(
  buildOutput,
) {
  const errors =
    Array.from(
      buildOutput.matchAll(
        /AccidentReconstructionEditor\.tsx:\d+:\d+\s*-\s*error\s+TS6133:\s*'([^']+)'\s+is declared but its value is never read\./g,
      ),
    );
  ...
  for (const match of errors) {
    ...
    console.log(
      `AUTO-CLEAN unused editor import: ${name}`,
    );
  }
...
if (
  build.status !== null &&
  build.status !== 0 &&
  repairUnusedEditorIconImports(
    build.output,
  )
) {
  console.log("");
  console.log(
    "Retrying build after safe unused-import cleanup...",
  );
...
if (
  ...
  console.error(
    `Full compiler output kept at: ${path.relative(
      root,
      buildLogPath,
    )}`,
  );
  ...
  console.error(
    "Restoring clean pre-V5 timeline state...",
  );
  ...
  console.error("");
  console.error(
    "The build log was intentionally preserved after rollback.",
  );
...
console.log("");
console.log(
  "RoadSafe Screen Timeline Component V5 installed successfully.",
);
console.log("");
console.log(
  "Architecture:",
...
console.log(
  "  node revoke-screen-timeline-component-v5.mjs",
);
