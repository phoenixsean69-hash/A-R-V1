...
export type MovementPathInterpolation =
  | "Auto"
  | "Bezier";
...
export interface MovementPathPoint {
  ...
  /**
   * Manual approach-path interpolation for the segment STARTING at this point.
   *
   * Auto = RoadSafe's existing smooth route solver.
   * Bezier = investigator-controlled cubic Bezier using bezierOut on this
   * point and bezierIn on the next authored point.
   *
   * These controls affect route geometry only. They never change participant
   * physical dimensions, mass, evidence positions, or visual scale.
   */
  pathInterpolation?: MovementPathInterpolation;

  /** Absolute scene-space incoming Bezier control handle (0-100 coordinates). */
  bezierIn?: ReconstructionPosition;

  /** Absolute scene-space outgoing Bezier control handle (0-100 coordinates). */
  bezierOut?: ReconstructionPosition;
}
...
export type ReconstructionDynamicsEngine =
  | "RoadSafe Legacy 2D"
  | "Rapier 3D Deterministic";

export type ParticipantDamageSeverity =
  | "Minor"
  | "Moderate"
  | "Severe"
  | "Critical";

export interface ParticipantVisualDamageState {
  participantId: string;
  collisionEventId: string;
  timeSeconds: number;
  contactZone: ParticipantImpactContactZone;
  severity: ParticipantDamageSeverity;

  /**
   * Simplified visual crush depth used only to drive the 3D damage cue.
   * This is NOT a validated structural crash/damage calculation.
   */
  crushDepthMetres: number;
...
export type SceneObjectInteractionKind =
  | "Pothole Jolt"
  | "Road Crack Jolt"
  | "Puddle Grip Loss"
  | "Oil Slip"
...
export interface SceneObjectInteractionEvent {
  id: string;
  timeSeconds: number;
  participantId: string;
  sceneObjectId: string;
  sceneObjectType: SceneObjectType;
  kind: SceneObjectInteractionKind;

  /** Approximate scene-space interaction point used for synchronized effects. */
  contactPosition?: ReconstructionPosition;

  /** Horizontal participant speed at the interaction instant. */
  speedKmh: number;

  /** 0..1 normalized visual/physics intensity. */
  intensity: number;

  /** Surface-zone multiplier applied to braking/grip where relevant. */
  gripMultiplier?: number;

  /** One-shot horizontal speed multiplier for bumps/soft impacts. */
  speedMultiplier?: number;

  /** Vertical launch/jolt applied by the dynamics layer, in m/s. */
  verticalImpulseMps?: number;

  /** Signed lateral/yaw disturbance used for deterministic slip response. */
  lateralDeflectionDegrees?: number;

  /** Contact impulse for dynamic object impacts when available. */
  impulseMagnitudeNs?: number;

  source: "Rapier Scene Object Interaction V1";
}

export interface SceneObjectDynamicsSample {
  timeSeconds: number;
  position: ReconstructionPosition;
...
export interface SceneObjectDynamicsState {
  sceneObjectId: string;
  sceneObjectType: SceneObjectType;
  mode: "Dynamic Body";
  participantIds: string[];
...
export interface RapierDynamicsDiagnostics {
  engine: "Rapier 3D Deterministic";
  packageName: "@dimforge/rapier3d-deterministic-compat";
  packageVersion: "0.19.3";
  fixedTimeStepSeconds: number;
  maxCcdSubsteps: number;
  dynamicParticipantBodies: number;
  fixedSceneObjectColliders: number;
  dynamicSceneObjectBodies?: number;
  surfaceZoneObjects?: number;
  collisionStarts: number;
  participantParticipantContacts: number;
  participantObjectContacts: number;
  maximumContactForceNewtons: number;
  firstContactTimeSeconds?: number;

  /**
   * Exact first relevant Rapier rigid-body contact time for each participant.
   * Used to synchronize visual impact/knockdown with the participant's actual
   * collider contact instead of the earlier forensic/swept estimate.
   */
  participantFirstContactTimesSeconds?: Record<
    string,
    number
  >;

  takeoverTimeSeconds: number;
  generatedPathPoints: number;
  simulatedDurationSeconds: number;
  usedForPostImpactTrajectories: boolean;

  /**
   * V1.2: vehicle/two-wheeler colliders follow their actual asset/physics
   * body footprint instead of the old collision-radius envelope.
   */
  participantColliderModel?:
    "Asset-matched rounded body";
}
...
export interface PhysicsSimulationSummary {
  ...
  /**
   * RoadSafe owns the forensic calculation/audit layer. When present,
   * dynamicsEngine identifies the rigid-body engine that generated the
   * canonical hidden post-impact playback trajectory.
   */
  dynamicsEngine?: ReconstructionDynamicsEngine;
  dynamicsEngineVersion?: string;
  rapierDynamics?: RapierDynamicsDiagnostics;

  /**
   * Participant-specific visual deformation cues derived only after a
   * real Rapier body contact. They are intentionally separate from the
   * forensic kinematics and must not be reported as structural crash analysis.
   */
  participantVisualDamage?: ParticipantVisualDamageState[];

  /**
   * Rapier-driven Road Hazard interactions. Surface zones alter participant
   * motion; dynamic scene objects also receive their own playback trajectory.
   */
  sceneObjectInteractions?: SceneObjectInteractionEvent[];
  sceneObjectDynamics?: SceneObjectDynamicsState[];
