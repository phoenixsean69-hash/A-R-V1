...
const statePath = path.join(
  backupRoot,
  "last-scene-collection-hq-dragdrop-v6.json",
);

const buildLogPath = path.join(
  backupRoot,
  "scene-collection-hq-dragdrop-v6-build.log",
);
...
function removeNamedImportIfImportOnly(
  source,
  identifier,
) {
  if (
...
/* ------------------------------------------------------------------ */
/* Refactor cleanup: remove imports made obsolete by the new browser.   */
/* ------------------------------------------------------------------ */

{
  const cleanup =
    removeNamedImportIfImportOnly(
      editor,
  ...
  if (cleanup.changed) {
    console.log(
      "CLEANED stale editor import: getParticipantAssetDefinition",
    );
  }
}
...
/* ------------------------------------------------------------------ */
/* Full build + one safe import-only TS6133 repair pass.               */
/* ------------------------------------------------------------------ */
...
function safelyRepairUnusedImportsFromBuild(
  compilerOutput,
) {
  const matches =
    Array.from(
      compilerOutput.matchAll(
        /([A-Za-z]:[\\/][^(\r\n]+|src[\\/][^(\r\n]+)\((\d+),(\d+)\):\s*error\s+TS6133:\s*'([^']+)'\s+is declared but its value is never read\./g,
      ),
    );
  ...
  for (
    ...
    console.log(
      `AUTO-CLEAN TS6133 import: ${path.relative(
        root,
        fullPath,
      )} -> ${identifier}`,
    );
...
const repairedImports =
  firstBuild.result.status !== 0
    ? safelyRepairUnusedImportsFromBuild(
        firstBuild.compilerOutput,
      )
    : 0;
...
if (
  ...
  console.log("");
  console.log(
    `Retrying build after ${repairedImports} safe unused-import cleanup(s)...`,
  );

  finalBuild =
    runProjectBuild(
      "after TS6133 import cleanup",
    );
}

const buildOutput =
  [
    "RoadSafe Blender Scene Collection HQ Drag/Drop V6",
    "=================================================",
    "",
    firstBuild.output,
    repairedImports > 0
...
console.log("");
console.log(
  "RoadSafe Blender Scene Collection HQ Drag/Drop V6 installed successfully.",
);
