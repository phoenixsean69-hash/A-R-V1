...
const statePath = path.join(
  backupRoot,
  "last-scene-collection-hq-dragdrop-v2.json",
);

const buildLogPath = path.join(
  backupRoot,
  "scene-collection-hq-dragdrop-v2-build.log",
);
...
function ensureNamedTypeImport(
  ...
  if (!match) {
    ...
    const statement = `import type { ${symbol} } from "${modulePath}";\\n`;

    return insertionPoint >= 0
      ? source.slice(0, insertionPoint + 2) +
          statement +
          source.slice(insertionPoint + 2)
      : statement + source;
  }
  ...
  if (closingIndex < 0) {
    fail(`Could not patch type import ${symbol} from ${modulePath}.`);
  }

  const beforeClosing = full.slice(0, closingIndex).replace(/\\s*$/, "");
  const replacement = `${beforeClosing},\\n  ${symbol},\\n${full.slice(closingIndex)}`;

  return source.replace(full, replacement);
}

function ensurePendingPlacementAssetId(source) {
  if (source.includes("assetId?: ReconstructionParticipantAssetId;")) {
    return source;
  }

  const pattern = /export interface PendingParticipantPlacement\\s*\\{([\\s\\S]*?)\\n\\}/m;
  const match = source.match(pattern);

  if (!match) {
    fail("Could not locate PendingParticipantPlacement interface.");
  }

  const full = match[0];
  const closingIndex = full.lastIndexOf("}");
  const replacement =
    full.slice(0, closingIndex) +
...
/* V2 patches by imported module/symbol, not line order/formatting.     */
/* ------------------------------------------------------------------ */
...
const buildOutput = [
  "RoadSafe Blender Scene Collection HQ Drag/Drop V2",
  "=================================================",
...
console.log("");
console.log(
  "RoadSafe Blender Scene Collection HQ Drag/Drop V2 installed successfully.",
);
...
console.log(
  "  node revoke-scene-collection-hq-dragdrop-v2.mjs",
);
