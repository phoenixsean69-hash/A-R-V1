...
import {
  ...
  type SceneObjectInteractionEvent,
...
import {
  getRecentSceneObjectInteraction,
  getSceneObjectDynamicsSampleAtTime,
} from "../../utils/reconstructionSceneObjectDynamics";
import {
...
function getParticipantRapierContactTime(
  ...
  /*
   * New Road Hazard dynamics can create a lightweight debris/branch contact
   * before the primary crash. When per-participant primary contact data exists
   * but this participant has no entry, do NOT fall back to that earlier global
   * contact and accidentally trigger the main crash/knockdown animation.
   */
  if (
    diagnostics
      ?.participantFirstContactTimesSeconds
...
function createSceneObjectInteractionEffectRoot(
  object:
  ...
  root.userData.sceneObjectInteractionEffect =
    true;
...
function applySceneObjectInteractionEffect(
  root: THREE.Group,
  object:
    ReconstructionSceneObject,
  event:
...
function applyParticipantRoadHazardVisualPose(
  entry:
    ParticipantRenderEntry,
  event:
    SceneObjectInteractionEvent | null,
  ...
  const signedDeflection =
    event.lateralDeflectionDegrees ??
    0;
  ...
  useEffect(() => {
      ...
      reconstruction.sceneObjects
        ...
        .forEach((object) => {
          if (object.tracePoints && object.tracePoints.length > 1) {
            ...
            const effectRoot =
              createSceneObjectInteractionEffectRoot(
                object,
              );
    ...
    const animate = (now: number) => {
      ...
      participantEntries.forEach((entry) => {
        ...
        const recentRoadHazardInteraction =
          effectiveShowPhysics
            ? getRecentSceneObjectInteraction(
                sceneObjectInteractions,
                {
        ...
        applyParticipantRoadHazardVisualPose(
          entry,
          recentRoadHazardInteraction,
          timeRef.current,
        );
        ...
        (entry) => {
          ...
          if (
            ...
            if (
              ...
              entry.holder.position.copy(
                worldPosition(
                  dynamicsSample.position,
                  width,
                  height,
                  dynamicsSample.verticalMetres,
                ),
              );

              entry.holder.quaternion.set(
                dynamicsSample
          ...
          applySceneObjectInteractionEffect(
            entry.effectRoot,
            object,
            recentInteraction,
            timeRef.current,
      ...
      const primaryRapierContactTimes =
        Object.values(
          reconstruction
            .lastPhysicsSimulation
            ?.rapierDynamics
      ...
      const impactTime =
        primaryRapierContactTimes.length >
          0
          ? Math.min(
              ...primaryRapierContactTimes,
            )
          : participantImpact
              ?.timeSeconds;
